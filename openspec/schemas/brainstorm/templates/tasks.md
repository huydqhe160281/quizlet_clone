...
<!--
  Break tasks into groups by component.
  Follow TDD: RED (Write) → RED (Verify) → GREEN → REFACTOR.
  Reference real file paths and struct names — verify via code_context.
  Run code_impact on each modified symbol — create update tasks for d=1 callers.
-->
...
<!-- If design-brief's Documentation Impact lists affected files, create explicit tasks. -->
