# Specification: Set & Card Import

## Capability Summary
Người dùng có thể import bộ flashcard và cards từ file CSV hoặc JSON trong một thao tác atomic, với preview và báo lỗi theo từng dòng.

## Layer Map

| Layer | Component | Symbol / Path |
|---|---|---|
| UI | Import wizard | `src/app/(app)/sets/import/page.tsx` |
| UI | Components | `src/features/sets/components/ImportSetWizard.tsx` |
| API | Route Handler | `src/app/api/v1/sets/import/route.ts` |
| Service | Import | `src/server/services/import.service.ts` |
| Validation | Schemas | `src/features/sets/schemas/import.schema.ts` |
| DB | Tables | `FlashcardSet`, `Flashcard` (transaction) |
...
- Anki `.apkg` import
- Export / download sets
- Import with image URLs in CSV (defer V2.1)
