# Specification: Spaced Repetition

## Capability Summary
Hệ thống tính toán và quản lý lịch ôn tập tối ưu cho mỗi card theo thuật toán SM-2, giúp người dùng ôn đúng thời điểm trước khi quên.

## Layer Map

| Layer | Component | Symbol / Path |
|---|---|---|
| UI | Study page | `src/app/(app)/study/page.tsx` — SM-2 due cards |
| UI | Components | `src/features/study/components/shared/GradeButtons.tsx` |
...
## Out of Scope
- FSRS4 algorithm (future enhancement)
- ML-based parameter optimization per user
- Cross-set SM-2 (cards from different sets treated as one queue)
- Forgetting index configuration per user
